...
def load_expression_table(X, cfg, **kwargs):
    ...
    data_loader = torch.utils.data.DataLoader(X, batch_size=cfg['minibatch_size'], shuffle=True, drop_last=True, **kwargs)
    return data_loader
...
class Encoder(nn.Module):
    ...
    def __init__(self, n_features, cfg):
        ...
        if cfg['use_batchnorm']:
            modules = [
                nn.Linear(n_features, cfg['layers'][0]),
                nn.BatchNorm1d(cfg['layers'][0]),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Linear(cfg['layers'][0], cfg['layers'][1]),
                nn.BatchNorm1d(cfg['layers'][1]),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Linear(cfg['layers'][1], cfg['layers'][2]),
                nn.BatchNorm1d(cfg['layers'][2]),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Linear(cfg['layers'][2], cfg['latent_size']),
                nn.Identity()
            ]
        else:
            modules = [
...
def loss_reconstruction(recon, x):
    """
...
def train(X, mode='BatchCorrection', cfg={}, device=torch.device('cpu')):
    """
    ...
    train_data = load_expression_table(X, cfg, **kwargs)

    print(f'Training the {mode} model for 1 datasets')
    training_losses = []
    for step, batch in enumerate(cycle(train_data)):
        ...
        # reconstruction loss
        recon_loss = loss_reconstruction(recon, x)
...
def output_activations(X, mode='BatchCorrection', cfg={}, data=None):
    """
    ...
    x = torch.tensor(asinh(X), dtype=torch.float)
    embed = encoder(x)
    cl = cluster(embed)
    recon = decoder(cl)

    results_embedded = pd.DataFrame(embed.detach().numpy(), columns=[f'Dim{i+1}' for i in range(cfg['latent_size'])])
    results_reconstructed = pd.DataFrame(sinh(recon.detach().numpy()), columns=data.var.index)
    all_cl_activations = cl.detach().numpy()

    n_clusters, clusters = get_clusters(all_cl_activations, results_embedded,
            binmin=cfg['binmin'], max_clusters=cfg['max_clusters'],
            merge_k_nearest=cfg['merge_k_nearest'])

    split_indices = np.cumsum(np.array(len(X)))[:-1]
...
if __name__ == '__main__':
    ...
    train(X, mode='Clustering', cfg=cfg, device=device)

    # get clusters and output
    results_clusters = output_activations(X, mode='Clustering', cfg=cfg, data=data)
